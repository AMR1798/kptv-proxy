...
func (fm *FilterManager) GetOrCreateFilter(source *config.SourceConfig) *CompiledFilter {
	...
	if source.SeriesIncludeRegex != "" {
		...
		if err != nil {
			logger.Error("{filter - GetOrCreateFilter} failed to compile SeriesIncludeRegex '%s': %v", source.SeriesIncludeRegex, err)
		} else {
	...
	if source.SeriesExcludeRegex != "" {
		...
		if err != nil {
			logger.Error("{filter - GetOrCreateFilter} failed to compile SeriesExcludeRegex '%s': %v", source.SeriesExcludeRegex, err)
		} else {
	...
	if source.VODIncludeRegex != "" {
		...
		if err != nil {
			logger.Error("{filter - GetOrCreateFilter} failed to compile VODIncludeRegex '%s': %v", source.VODIncludeRegex, err)
		} else {
	...
	if source.VODExcludeRegex != "" {
		...
		if err != nil {
			logger.Error("{filter - GetOrCreateFilter} failed to compile VODExcludeRegex '%s': %v", source.VODExcludeRegex, err)
		} else {
